...
namespace PreTrain
{
    class AITag
    {
        public static void StartTag()
        {
            ...
            data.RemoveAll(x => x.Length < 5);
            data.RemoveAll(x => x.Length > 200);
            ...
            Parallel.ForEach(data, options, (x) =>
            {
                ...
                try
                {
                    result = completions.Ask('"' + x + '"', TestedModel.DeepSeek.APIUrl, apikey)!.GetMessageContent()!;
                    if (string.IsNullOrEmpty(result))
                        return;
                    result = result.Replace("```json", "").Replace("```", "");
                    result = result.Split('}', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries)[0] + '}';
                    TrainData tdata = JsonConvert.DeserializeObject<TrainData>(result, jsonsetting)!;
                    tdata.Text = x;
                    output.Add(tdata);
                    ...
                    if (processed % 100 == 0)
                    {
                        Console.WriteLine($"已处理 {processed}/{data.Count}");
